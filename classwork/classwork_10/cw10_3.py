...
Evaluation method name: def grade_question_cw10_3_answer
"""
...
class CW10_3Evaluator(BaseEvaluator):
    ...
    Justify your selection, i.e. explain why did you choose this form of ANOVA-related group of tests (5 points).
    Step 2. State hypotheses. If it needs the math form, apply it. If it doesn't, explain why. (5 points).
    ...
    def check_required_elements(self, student_answer: str) -> dict:
        ...
        # Method selection
        if re.search(r'anova|one[\s-]?way|welch|kruskal[\s-]?wallis|brown[\s-]?forsythe', text_lower):
        ...
        # Justification
        if re.search(r'because|since|due to|assumption|normal|variance|homogeneity', text_lower):
        ...
        # Hypotheses
        if re.search(r'h[0o]|h[1a]|null\s*hypothesis|alternative\s*hypothesis|μ|mu', text_lower):
        ...
        if re.search(r'α|alpha|df|f\s*=|statistic|p[\s-]?value|reject|fail to reject|conclude', text_lower):
    ...
    def grade_question_cw10_3_answer(self, student_answer: str, test_mode: bool = False):
...
{{
  ...
  "vibe": "..."
    ...
    def print_grading_results(self, grading):
        ...
        print("=" * 60)
        print("GRADING RESULTS - CLASSWORK 10.3")
        print("=" * 60)
...
if __name__ == "__main__":
    evaluator = CW10_3Evaluator()

    from config import InputHandler
    input_handler = InputHandler()

    student_answer = input_handler.collect_and_validate_input(
        question_name="CLASSWORK 10.3",
        question_description="ANOVA Step System",
        min_length=10
    )

    if student_answer:
        grading = evaluator.grade_question_cw10_3_answer(student_answer)
