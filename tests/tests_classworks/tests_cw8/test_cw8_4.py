...
SAVE LOCATION: tests/tests_classworks/tests_cw8/test_cw8_4.py

RUN WITH: pytest tests/tests_classworks/tests_cw8/test_cw8_4.py -v

Regression tests for the deterministic Component 1 (Formatting) override
in classwork/classwork_8/cw8_4.py's grade_cw8_4_answer method.

Verifies that component_1_score is forced from
check_formatting_elements_type2's elements_found (task_description,
autoformatting), not left to the LLM to assign - which previously caused
random/incorrect scoring on the formatting component regardless of the
actual student answer.

Only the network call (call_groq_api) is mocked; everything else,
including the override logic itself, runs against the real code.
If the override is ever removed or broken, these tests fail.
"""
...
# tests/tests_classworks/tests_cw8/test_cw8_4.py -> project root is 3 levels up
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../..")))

from classwork.classwork_8.cw8_4 import CW8_4Evaluator


@pytest.fixture
def evaluator():
    """Deterministic fixture providing a clean evaluator instance for every test."""
    return CW8_4Evaluator()
...
def _mock_llm_json(component_1_score, component_1_task_score, component_1_autoformat_score):
    ...
    to verify the deterministic override in grade_cw8_4_answer corrects it
...
def test_component1_forced_when_td_absent(evaluator):
    ...
    "please calculate" is absent from the student answer. The deterministic
    ...
    with patch.object(evaluator, "call_groq_api", return_value=fake_response):
        result = evaluator.grade_cw8_4_answer(student_answer)
...
def test_component1_forced_when_autoformat_violated(evaluator):
    ...
    """
    student_answer = "please calculate\n\n- first item\n- second item"
