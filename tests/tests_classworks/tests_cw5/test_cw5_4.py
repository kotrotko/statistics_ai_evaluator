...
SAVE LOCATION: tests/tests_classworks/tests_cw5/test_cw5_4.py

RUN WITH: pytest tests/tests_classworks/tests_cw5/test_cw5_4.py -v

Regression tests for the deterministic Component 1 (Formatting) override
in classwork/classwork_5/cw5_4.py's grade_cw5_4_answer method.
...
# tests/tests_classworks/tests_cw5/test_cw5_4.py -> project root is 3 levels up
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../..")))

from classwork.classwork_5.cw5_4 import CW5_4Evaluator


@pytest.fixture
def evaluator():
    """Deterministic fixture providing a clean evaluator instance for every test."""
    return CW5_4Evaluator()
...
def _mock_llm_json(component_1_score, component_1_task_score, component_1_autoformat_score):
    ...
    to verify the deterministic override in grade_cw5_4_answer corrects it
...
def test_component1_forced_when_td_absent(evaluator):
    ...
    "what proportion" is absent from the student answer. The deterministic
    ...
    with patch.object(evaluator, "call_groq_api", return_value=fake_response):
        result = evaluator.grade_cw5_4_answer(student_answer)
...
def test_component1_forced_when_autoformat_violated(evaluator):
    ...
    """
    student_answer = "what proportion\n\n- first item\n- second item"
